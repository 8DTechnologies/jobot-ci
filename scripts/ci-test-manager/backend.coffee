...
class CITestManagerBackendSingleton
  ...
  class CITestManagerBackend extends EventEmitter
      ...
      @persist ( storage )->
        ...
        for test, detail of failedtests
          if previousFailedTest?[test]
            ...
            assignee =( _.findLast storage.projects, (project) -> project.failedtests?[test]?.assigned)?.failedtests?[test]?.assigned
            ...
            currentFailedTest[test] =
              ...
              builds: {}
            if assignee
              currentFailedTest[test].assigned = assignee
              currentFailedTest[test].assignedDate = moment()
    ...
    #
    assignTests: ( testnames, user ) ->
      # Convert projects to array of keys
      projects = Object.keys(@getProjects())
      @persist ( storage )->
        for project in projects
          for testname in testnames
            # Check if testname exists and if not, throw exception
            if storage.projects[project]?.failedtests?[testname]
             ...
             console.log("No test #{testname} for project #{project}")
