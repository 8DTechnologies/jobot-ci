...
#   process.env.HUDSON_TEST_MANAGER_ASSIGNMENT_TIMEOUT_IN_MINUTES=15
...
#
# Notes:
# This script require a json file auth.json at the root of the hubot project. auth.json ={"teamcity":{user:"value","password":"value"}}
...
class CITestManager
  ...
  handleAssignTest: ( msg ) ->
    ...
    userName = msg.envelope.user.privateChatJID.split('@')[0]

    project = msg.match[2] or  @getLastAnnouncement( "#{msg.envelope.user.room}" )?.projectname
    unless  project
      console.log(JSON.stringify(@getLastAnnouncement( userName),null,4))
      project = @getLastAnnouncement( userName)?.projectname
    ...
    try
      ...
      broadcastInPrivate = false
      unless lastannouncement
        if msg.envelope.user.type is 'chat'
          lastannouncement = @getLastAnnouncement userName
          broadcastInPrivate = true
      for index, testname of tests
        @robot.logger.info index, testname
        unless isNaN( testname )
          ...
          if lastannouncement
            ...
            assignedInPrivateTests.push tests[index]
          else
      ...
      @backend.assignTests  tests, user
      if broadcastInPrivate
        message = new Xmpp.Element( 'message', {} )
        body =  message.c( 'html', {xmlns: 'http://jabber.org/protocol/xhtml-im'} ).c( 'body', {xmlns: 'http://www.w3.org/1999/xhtml'} )
        body.t("Tests assigned from #{project}.").c('br')
        if lastannouncement
          i =0
          for test in assignedInPrivateTests
            body.t("#{++i} - ").c( 'a', {href: @backend.getProjects()[project].failedtests[test].url} ).t( test ).up().c('br')
        else
          body.t("#{++i} - ").c( 'a', {href: @backend.getProjects()[project].failedtests[testname].url} ).t( testname ).up().c('br')
        @sendPrivateMesssage user, message
  ...
  handleShowTestReportForProject: ( msg ) ->
    ...
    if msg.envelope.user.type == 'groupchat'
      ...
      if msg.envelope.user.room is projectRoomName
      ...
      @storeAnnouncement userName, projectname, announcement
  ...
  handleShowTestAssignedToMe: ( msg ) ->
    ...
    message = new Xmpp.Element( 'message', {} )
    body =  message.c( 'html', {xmlns: 'http://jabber.org/protocol/xhtml-im'} ).c( 'body', {xmlns: 'http://www.w3.org/1999/xhtml'} )
    body.t("Tests assigned to #{user}.").c('br')
  ...
  processNewTestResult: ( projectname, buildname, fixedTests, newFailedTest, currentFailedTest ) =>
    ...
    sending = false
    if (Object.keys( fixedTests ).length !=  0) and displayFixed
    ...
    @sendGroupChatMesssage roomname, message if sending
